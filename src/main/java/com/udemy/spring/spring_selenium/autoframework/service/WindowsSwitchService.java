package com.udemy.spring.spring_selenium.autoframework.service;
...
public class WindowsSwitchService {
    ...
    public void switchByIndex(final int index) {
        WebDriver driver = this.ctx.getBean(WebDriver.class);
        String[] handlers = driver.getWindowHandles().toArray(new String[0]);
        driver.switchTo().window(handlers[index]);
    }
}
